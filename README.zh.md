...
Hosted tools 是单次请求的能力，不是服务商级默认值。只有调用方明确传入 `tools` 列表时，插件才会用同一次决策完成上下文去重和 payload 注入。自动审批、标题生成等不带 `tools` 的辅助纯文本调用会保持纯文本；即使服务商序列化器随后生成空的 `tools: []` 字段，也不会被塞入搜索或出图工具。
